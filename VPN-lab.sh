...
gcloud compute --project=warm-classifier-721 instances create instance-2 --zone=europe-west1-b --machine-type=f1-micro --subnet=subnet-b --network-tier=PREMIUM --maintenance-policy=MIGRATE --service-account=778083151326-compute@developer.gserviceaccount.com --scopes=https://www.googleapis.com/auth/cloud-platform --tags=http-server,https-server --image=debian-9-stretch-v20190514 --image-project=debian-cloud --boot-disk-size=10GB --boot-disk-type=pd-standard --boot-disk-device-name=instance-1

gcloud compute --project=warm-classifier-721 firewall-rules create vpn-network-2-allow-http --direction=INGRESS --priority=1000 --network=vpn-network-2 --action=ALLOW --rules=tcp:80 --source-ranges=0.0.0.0/0 --target-tags=http-server

gcloud compute --project=warm-classifier-721 firewall-rules create vpn-network-2-allow-https --direction=INGRESS --priority=1000 --network=vpn-network-2 --action=ALLOW --rules=tcp:443 --source-ranges=0.0.0.0/0 --target-tags=https-server

#create firewall rules to allow tcp 22 and icmp for second VM
gcloud compute --project=warm-classifier-721 firewall-rules create allow-icmp-ssh-network-2 --direction=INGRESS --priority=1000 --network=vpn-network-2 --action=ALLOW --rules=tcp:22,icmp --source-ranges=0.0.0.0/0
